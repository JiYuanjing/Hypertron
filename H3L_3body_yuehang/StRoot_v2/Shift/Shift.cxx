...
Int_t Shift::Centrality(int gRefMult )
{
    ...
    int centFull[9]={3,6,11,18,29,44,64,93,112};
...
Int_t Shift::Make() {
    ...
    for (Int_t itr=0;itr<nTrack;itr++) {
        ...
        if(!isGoodTrackSooraj(ptrk))  continue;
...
//    if(centnumber > 8 || centnumber < 0) return kStOK;
    */
    ...
    centnumber  = mPileupTool->get_centrality9();  if (centnumber<0) return 0;
    ...
    for(int j=0; j<7; j++)
    {
        ...
        {
            psi1shift_EPD[j] +=
                (2.0/(i+1)) * ( -pp_EPDshiftpar_sin[j]->GetBinContent(centnumber+1, i+1, runindex+1)*cos(1.0*(i+1)*psi1recen_EPD[j]) + pp_EPDshiftpar_cos[j]->GetBinContent(centnumber+1, i+1, runindex+1)*sin(1.0*(i+1)*psi1recen_EPD[j]) );
        }
    }
    for(int i=0; i<7; i++)
    {
