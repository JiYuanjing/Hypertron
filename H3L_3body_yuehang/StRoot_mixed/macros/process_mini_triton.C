...
int _condor;
int _cuts;
void process_mini_triton(int condor=0){
	...
	if(_condor==0){
	fin.open("/star/u/yhleung2/pwg/46.tpcrs_3p85gev/triton_new_t0/triton_x.list");
	strcpy (ofile, "triton_x.root");
	nChains = 600;
	}
		
	if(_condor==1){
        fin.open("/star/u/yhleung2/pwg/46.tpcrs_3p85gev/triton_new_t0/triton.list");
        strcpy (ofile, "triton.root");
        nChains = 450;
        }
...
*/
	...
	for(int i=0;i<mNMcTrack;i++){
		if(mMcTracksmGeantId[i]!=46) continue;
