...
void book_histo();
void write_histo();
void process_tree_tmva_ks_kf_vn_v4_test(int condor=0,int evt_plane=0, int batch=0, int cut=1){
...
//cuts
	...
	TFile  *fgpt_eff = new TFile(Form("ks_vn_eff_v0_cut%d.root",_cut),"READ");
...
strcpy (ofile,Form("run18_3gev_ks_kf_flow_weighted_evt%d_cut%d_nbins%d_test.root",_evt_plane,_cut,ncutptbins));
nChains = 1215;
}else{
fin.open("/star/u/yhleung2/pwg/33.standard_reco/taxi_ks_ana_flow_rotate_loose/filelist.txt");
strcpy (ofile,Form("run18_3gev_ks_kf_flow_rotate_weighted_evt%d_cut%d_nbins%d_test.root",_evt_plane,_cut,ncutptbins));
nChains = 1217;
}
...
if(bparticleid>0){
  ...
  h_om_mass[cent_label][pt_cut_label][rap_label]->Fill(bparticlemass,1./eff_weight);
