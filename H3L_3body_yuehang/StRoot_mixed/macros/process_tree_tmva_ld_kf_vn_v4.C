...
void book_histo();
void write_histo();
void process_tree_tmva_ld_kf_vn_v4(int condor=0,int evt_plane=0, int batch=0, int cut=1){
...
if(!_mb){
strcpy (ofile,Form("run18_3gev_ld_kf_flow_new_weighted_evt%d_cut%d.root",_evt_plane,_cut));
}else{
strcpy (ofile,Form("run18_3gev_ld_kf_flow_new_weighted_evt%d_cut%d_mb.root",_evt_plane,_cut));
}
nChains = 1222;
}else{
fin.open("/star/u/yhleung2/pwg/33.standard_reco/taxi_ld_ana_flow_vn_rotate_loose/filelist.txt");
if(!_mb){
strcpy (ofile,Form("run18_3gev_ld_kf_flow_rotate_new_weighted_evt%d_cut%d.root",_evt_plane,_cut));
}else{
strcpy (ofile,Form("run18_3gev_ld_kf_flow_rotate_new_weighted_evt%d_cut%d_mb.root",_evt_plane,_cut));
}
nChains = 1216;
}
}else{
