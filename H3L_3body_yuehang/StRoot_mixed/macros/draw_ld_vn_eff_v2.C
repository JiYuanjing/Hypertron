...
TH2F *g_pt_fine2_wgt;
int _cut_mode;
void draw_ld_vn_eff_v2(int cut_mode){
...
TFile *f0;
f0 = new TFile(Form("run18_3gev_ld_hist_mu_1x_vn_newreweight_centa0_cut%d_v2.root",_cut_mode),"READ");

h_mass_pt_fine = (TH3F*)f0->Get("h_mass_pt_wgt");
...
TH2F *h_pt_fine2;
 ...
 h_pt_fine = new TH2F("h_pt_fine",""          ,20,-1.0,1.0,50,0,5);
 h_pt_wgt_fine = new TH2F("h_pt_wgt_fine",""  ,20,-1.0,1.0,50,0,5);
...
TH2F *h_pt_eff_fine;
h_pt_eff_fine = (TH2F*)h_pt_fine->Clone("h_pt_eff_fine");
...
for(int ix=1;ix<=h_pt_wgt_fine2->GetNbinsX();ix++ ){//rap
    for(int iy=1;iy<=h_pt_wgt_fine2->GetNbinsY();iy++ ){//pt
      h_mass_pt_wgt2_px = (TH1F*)h_mass_pt_fine2_wgt->ProjectionX("h_mass_pt_wgt2_px",ix,ix,iy,iy);
      counts = 0;
      for(int ibin=1;ibin<=h_mass_pt_wgt2_px->GetNbinsX();ibin++){
        bincenter = h_mass_pt_wgt2_px->GetBinCenter(ibin);
        bincontent = h_mass_pt_wgt2_px->GetBinContent(ibin);
        if( bincenter>(lambda_mass-3*sgm3) && bincenter<(lambda_mass+3*sgm3) )  {counts+=bincontent;}
      }
...
TFile *outhistfile = new TFile (Form("ld_vn_eff_v0_cut%d_v2.root",_cut_mode), "RECREATE");
outhistfile->cd();
