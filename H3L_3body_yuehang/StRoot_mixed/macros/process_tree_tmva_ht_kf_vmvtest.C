...
void process_tree_tmva_ht_kf_vmvtest(int condor=0, int cut_mode=0, int wgt=2, int batch=0){
...
//	t_quadr->SetParameters(2.06539, 0.0, -1.75299);//this is from fit to 3gevlambdadata
	...
	bolt0->SetParameter(0,0.20);
	bolt1->SetParameter(0,0.10);
	bolt2->SetParameter(0,0.06);
...
strcpy (ofile, Form("run18_3gev_ht_hist_mu_1x_14_newreweight_centa%d_wgt%d_cut%d_vtest.root",choose_cent,_wgt,_cut_mode));
nChains = 1126;
}
if(_condor==1){//data
...
nChains = 1222;
	...
	if(_batch==1){
   	...
   	strcpy (ofile,Form("run18_3gev_ht_hist_pico_rotate_0_bdt_10_centa%d_cut%d_v9.root",choose_cent,_cut_mode));
nChains = 1208;
	}
	if(_batch==2){
...
void book_histo()
{
  ...
  h_mass_pt = new TH3F("h_mass_pt","",1000,2.5,3.5,10,-1.0,1.0,50,0,5);
  ...
  h_mass_pt_wgt = new TH3F("h_mass_pt_wgt","",1000,2.5,3.5,10,-1.0,1.0,50,0,5);
