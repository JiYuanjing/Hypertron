...
void process_tree_tmva_3b_kf_vmv10(int condor=0, int cut_mode=0, int wgt=2, int batch=0){
...
fin.open("/star/u/yhleung2/pwg/33.standard_reco/taxi_3b_mu_manualvtxerr_new/filelist.txt");
//strcpy (ofile, Form("run18_3gev_3b_hist_mu_1x_30_newreweight_centa%d_wgt%d_cut%d_mve.root",choose_cent,_wgt,_cut_mode));

strcpy (ofile, Form("run18_3gev_3b_hist_mu_1x_35_newreweight_centa%d_wgt%d_cut%d_mve.root",choose_cent,_wgt,_cut_mode));
...
//        strcpy (ofile,Form("run18_3gev_3b_hist_pico_bdt_26_centa%d_cut%d.root",choose_cent,_cut_mode));//12: new finder
	...
	//strcpy (ofile,Form("run18_3gev_3b_hist_pico_bdt_33_centa%d_cut%d.root",choose_cent,_cut_mode));//30:onesided, 31:1sigma, 32: no cut
	        strcpy (ofile,Form("run18_3gev_3b_hist_pico_bdt_34_centa%d_cut%d.root",choose_cent,_cut_mode));//30:onesided, 31:1sigma, 32: no cut//30b onesided, fix the weighting, 34:no cut, ptcut
	      nChains = 1197;
	...
	strcpy (ofile,Form("run18_3gev_3b_hist_pico_rotate_0_bdt_30_centa%d_cut%d.root",choose_cent,_cut_mode));
	              nChains = 1191;
	}
	if(_batch==2){
...
fin.open("/star/u/yhleung2/pwg/33.standard_reco/taxi_3b_pico_rotate_tree_deuteron/filelist.txt");
        strcpy (ofile,Form("run18_3gev_3b_hist_pico_rotate_0_bdt_34_centa%d_cut%d.root",choose_cent,_cut_mode));
      nChains = 1207;
...
//		h_mass_pt->Fill(bparticlemass,brap,bpt);
	...
	float lambda_weight;
	//if(ptc_lambda.M()>1.11583-2*4.01078e-03 && ptc_lambda.M()<1.11583+2*4.01078e-03) lambda_weight=ld_gaus->Eval(ptc_lambda.M());
...
void book_histo()
{
  ...
  h_mass_pt_wgt = new TH3F("h_mass_pt_wgt","",1000,2.5,3.5,1,0,0.8,50,0,5);
  ...
  g_pt_wgt = new TH2F("g_pt_wgt","",1,0,0.8,50,0,5);
