...
//
const int ncutptbins = 5;
double ptcutbin[ncutptbins+1] = {0.0,0.4,0.8,1.2,1.6,4.0};
...
void book_histo();
void write_histo();
void process_tree_tmva_ks_kf_vn(int condor=0){
...
for(int icent=0;icent<ncentbins;icent++){
  for(int ipt=0;ipt<ncutptbins;ipt++){
h_om_mass[icent][ipt] = new TH1F(Form("h_om_mass[%d][%d]",icent,ipt), Form("h_om_mass[%d][%d]",icent,ipt), 1000,lomass,himass);
}
}
