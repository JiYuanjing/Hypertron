...
void process_tree_tmva_5l2_kf_vmv8(int condor=0, int cut_mode=0, int wgt=2, int batch=0){
...
//fin.open("/star/u/yhleung2/pwg/33.standard_reco/taxi_3b_mu_manualvtxerr/filelist.txt");
//strcpy (ofile, Form("run18_3gev_3b_hist_mu_1x_14_newreweight_centa%d_wgt%d_cut%d_mve.root",choose_cent,_wgt,_cut_mode));
//nChains = 255;

fin.open("/star/u/yhleung2/pwg/33.standard_reco/taxi_5l_mu/filelist.txt");
strcpy (ofile, Form("run18_3gev_5l_1x_14_newreweight_centa%d_wgt%d_cut%d_mve.root",choose_cent,_wgt,_cut_mode));
nChains = 93;
...
//   	strcpy (ofile,Form("run18_3gev_3b_hist_pico_bdt_11_centa%d_cut%d.root",choose_cent,_cut_mode));
        ...
        //strcpy (ofile,Form("run18_3gev_3b_hist_pico_bdt_21_centa%d_cut%d.root",choose_cent,_cut_mode));//12: new finder
        
	//fin.open("/star/u/yhleung2/pwg/33.standard_reco/taxi_5l_pico_tree_new_tfg/filelist.txt");
	//strcpy (ofile,Form("run18_3gev_5l2_hist_pico_bdt_21_centa%d_cut%d.root",choose_cent,_cut_mode));
	//nChains = 1222;
	
	fin.open("/star/u/yhleung2/pwg/33.standard_reco/taxi_5l_pico_tree_new_search_2/filelist.txt");
        strcpy (ofile,Form("run18_3gev_5l2_hist_pico_bdt_24_centa%d_cut%d.root",choose_cent,_cut_mode));
        nChains = 1222;
   	}
	if(_batch==1){
	...
	fin.open("/star/u/yhleung2/pwg/33.standard_reco/taxi_5l_pico_tree_rotate_he4/filelist.txt");
        strcpy (ofile,Form("run18_3gev_5l2_hist_pico_rotate_0_bdt_24_centa%d_cut%d.root",choose_cent,_cut_mode));
	nChains = 1222;
	}
	if(_batch==2){
	fin.open("/star/u/yhleung2/pwg/33.standard_reco/taxi_5l_pico_tree_rotate_proton/filelist.txt");
        strcpy (ofile,Form("run18_3gev_5l2_hist_pico_rotate_proton_bdt_24_centa%d_cut%d.root",choose_cent,_cut_mode));
        nChains = 1222;
	}
        if(_batch==3){
	fin.open("/star/u/yhleung2/pwg/33.standard_reco/taxi_5l_pico_tree_rotate_pion/filelist.txt");
        strcpy (ofile,Form("run18_3gev_3b_hist_pico_rotate_pion_bdt_24_centa%d_cut%d.root",choose_cent,_cut_mode));
	        nChains = 1219;
	}
        if(_batch==4){
	fin.open("/star/u/yhleung2/pwg/33.standard_reco/taxi_ht_pico_tree_newpid_rotate_3/filelist.txt");
        strcpy (ofile,Form("run18_3gev_ht_hist_pico_rotate_3_bdt_9_centa%d_cut%d.root",choose_cent,_cut_mode));
	}
	//fin.open("/star/u/yhleung2/pwg/33.standard_reco/taxi_ht_pico_tree_newpid_rotate_4/filelist.txt");
        //strcpy (ofile,Form("run18_3gev_ht_hist_pico_rotate_4_bdt_9_centa%d_cut%d.root",choose_cent,_cut_mode));
        if(_batch==5){
	fin.open("/star/u/yhleung2/pwg/33.standard_reco/taxi_ht_pico_tree_newpid_rotate_5/filelist.txt");        
        strcpy (ofile,Form("run18_3gev_ht_hist_pico_rotate_5_bdt_9_centa%d_cut%d.root",choose_cent,_cut_mode));
	}
        if(_batch==6){
	fin.open("/star/u/yhleung2/pwg/33.standard_reco/taxi_ht_pico_tree_newpid_rotate_6/filelist.txt");
        strcpy (ofile,Form("run18_3gev_ht_hist_pico_rotate_6_bdt_9_centa%d_cut%d.root",choose_cent,_cut_mode));
	}
        if(_batch==7){
        fin.open("/star/u/yhleung2/pwg/33.standard_reco/taxi_ht_pico_tree_newpid_rotate_7/filelist.txt");
        strcpy (ofile,Form("run18_3gev_ht_hist_pico_rotate_7_bdt_9_centa%d_cut%d.root",choose_cent,_cut_mode));
	}
//	nChains = 1030;
	nChains = 1170;
...
//	strcpy (ofile,Form("run18_3gev_hl_hist_pico_rotate_pion_loose_8_centa%d_cut%d.root",choose_cent,_cut_mode));
	...
	fin.open("/star/u/yhleung2/pwg/33.standard_reco/taxi_3b_pico_rotate_tree_0_0_0_rotate_proton/filelist.txt");
	strcpy (ofile,Form("run18_3gev_hl_hist_pico_rotate_proton_loose_8_centa%d_cut%d.root",choose_cent,_cut_mode));
	nChains = 994;
	}
...
htriton3_tree.SetBranchAddress("v_12_chi2primary", &v_12_chi2primary);
	...
	//ht_ldl_cut = 3;
	//ht_l_cut = 1;
	chi2primary_proton_cut = 0.0;
	chi2primary_pi_cut = 0.0;

	hl_chi2topo_cut = 5;
	hl_chi2ndf_cut = 5;
	//hl_ldl_cut = 3;
	//hl_l_cut = 1;
	chi2primary_h4_cut = 0.0;
	chi2primary_pi_cut = 0.0;
	}
...
//TLorentzVector ptc_he(bprotonpx,bprotonpy,bprotonpz,sqrt(bprotonpx*bprotonpx+bprotonpy*bprotonpy+bprotonpz*bprotonpz+3.72840015733*3.72840015733));
//TLorentzVector ptc_proton(bdpx,bdpy,bdpz,sqrt(bdpx*bdpx+bdpy*bdpy+bdpz*bdpz+0.93827208816*0.93827208816));
//TLorentzVector ptc_lambda = ptc_pion + ptc_proton;
//TLorentzVector ptc_pd = ptc_he + ptc_proton;
...
// bmcrap = mcptc.Rapidity() - ycm; 
	...
	if(ht_pl<5) continue;//no signal for ht_pl<6
	//TODO
...
//	if( brap<0.55  &&  bpt < 1.9*(brap-0.65)*(brap-0.65)+0.67) continue;
	...
	//
	//if(mass_02>1.11568-0.007 && mass_02<1.11568+0.009) continue;//17
//	if(mass_01>3.92 && mass_01<3.938) continue;
//        if(v_02_chi2primary<10) continue;	

	//if(ptc_lambda.M()>1.11568-0.006) continue;//19
	
	if(mass_12>4.675) continue;

	if(bparticleid==3007 && ((bismc==1&&_condor==0) || _condor==1 || _condor==2) ){
	//if(bparticleid==3004 && ( _condor==1 || _condor==2) ){

//        h_lambda->Fill(ptc_lambda.M());
	if(bparticlemass>mass_lo && bparticlemass<mass_hi && !_condor==0){

	//cout<<"mass_01:"<<mass_01<<" "<<mass_02<<" "<<mass_12<<endl;

        h_lambda->Fill(mass_02);
        h_hep->Fill(mass_12);
	h_h4l->Fill(mass_01);
...
//        if((_cut_mode==11|| _cut_mode==12||_cut_mode==13) && mvaValue<mvaCut) continue;	
...
//	weight *= cent_weight;
	...
	if(bparticleid==3007 && (bismc==1&&_condor==0) ){
	if(bparticlemass>mass_lo && bparticlemass<mass_hi){

//	cout<<"mass_01:"<<mass_01<<" "<<weight<<endl;

        h_lambda->Fill(mass_02,weight);
	h_h4l->Fill(mass_01,weight);
        h_hep->Fill(mass_12,weight);
...
if(bmcparticleid!=3007) continue;
...
void book_histo()
{
  ...
  h_mass_pt = new TH3F("h_mass_pt","",1000,4.5,5.5,100,-1.0,1.0,50,0,5);
  h_mass_pl = new TH3F("h_mass_pl","",1000,4.5,5.5,10,-1.0,1.0,500,0,100);

  h_mass_pt_wgt = new TH3F("h_mass_pt_wgt","",1000,4.5,5.5,100,-1.0,1.0,50,0,5);
  h_mass_pl_wgt = new TH3F("h_mass_pl_wgt","",1000,4.5,5.5,10,-1.0,1.0,500,0,100);

  h_mass_qa = new TH3F("h_mass_qa","",10,-2.0,0.0,50,0,5,1000,4.5,5.5);
...
h_h4l = new TH1F("h_h4l","",1000,3.5,4.5);
h_hep = new TH1F("h_hep","",1000,4.5,5.5);
