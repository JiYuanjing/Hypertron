...
void book_histo();
void write_histo();
void process_tree_tmva_ks_kf_vn_v4(int condor=0,int evt_plane=0, int batch=0, int cut=1){
...
strcpy (ofile,Form("run18_3gev_ks_kf_flow_weighted_evt%d_cut%d_nbins%d.root",_evt_plane,_cut,ncutptbins));
nChains = 1215;
}else{
fin.open("/star/u/yhleung2/pwg/33.standard_reco/taxi_ks_ana_flow_rotate_loose/filelist.txt");
strcpy (ofile,Form("run18_3gev_ks_kf_flow_rotate_weighted_evt%d_cut%d_nbins%d.root",_evt_plane,_cut,ncutptbins));
nChains = 1217;
}
