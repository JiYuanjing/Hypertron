//v17: change the binning for v2 rapidith
...
const int nrapbins_2 = 6;
float rapbin_2[nrapbins_2+1] = {-0.5,-0.25,0.0,0.25,0.5,0.75,1.0};
...
void book_histo();
void write_histo();
void process_tree_tmva_ld_kf_vn_v17(int condor=0,int evt_plane=0, int batch=0, int cut=1){
...
if(!_mb){
strcpy (ofile,Form("run18_3gev_ld_kf_flow_new_weighted_evt%d_cut%d_v17.root",_evt_plane,_cut));
}else{
strcpy (ofile,Form("run18_3gev_ld_kf_flow_new_weighted_evt%d_cut%d_v17_mb.root",_evt_plane,_cut));
}
nChains = 1222;
}else{
...
if(!_mb){
strcpy (ofile,Form("run18_3gev_ld_kf_flow_rotate_new_weighted_evt%d_cut%d_v17.root",_evt_plane,_cut));
}else{
strcpy (ofile,Form("run18_3gev_ld_kf_flow_rotate_new_weighted_evt%d_cut%d_v17_mb.root",_evt_plane,_cut));
}
nChains = 1222;
}
}else{
