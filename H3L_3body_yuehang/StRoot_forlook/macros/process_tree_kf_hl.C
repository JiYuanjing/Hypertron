...
const int nptbins = 1;
TTree *htriton_tree_pt[nptbins];
double ptbin[nptbins+1] =    {2.0,50.0};
...
void book_tree();
void write_tree();
void process_tree_kf_hl(int aa);
void process_tree_kf_hl(int data_or_sim){
	...
	t_quadr->SetParameters(4.71255e-01,0.00000e+00,1.79971e+00*1.5);
	t_quadr0->SetParameters(7.03832e-01,0.,9.60834e-01);
...
//		nChains = 393;
	...
	strcpy (ofile, "run18_3gev_tmva_hl_sig.root");
	nChains = 1268;
	}
	if(_data_or_sim==1){
		//rotation
		//fin.open("/star/u/yhleung2/pwg/33.standard_reco/taxi_ht_pico_tree_newpid_rotate_0/filelist_merge.txt");
		...
		strcpy (ofile,"run18_3gev_tmva_hl_bkg.root");
		nChains = 1208;
	}
	...
	if(bpl > ptbin[nptbins]) continue;
        if(bpl<2) continue;//no signal for ht_pl<2
        if(bpl>50) continue;//no signal for ht_pl<2
	//pre cuts
...
if( brap<0.7  &&  bpt < 2.25*(brap-0.80)*(brap-0.80)+0.5-0.029+0.1195 ) continue;
	...
	if(_data_or_sim==0 && bismc==1){
		...
		weight = rap_weight*pt_weight;
	        weight *= mc_weight;	
         	weight *= cent_weight;
//cout<<"weight:"<<rap_weight<<" "<<pt_weight<< " "<<mc_weight<<" "<<cent_weight<<endl;
