...
Short_t mSplitPairsmNAssocGl[MAXv0];
Short_t mMatchedPairsmNAssocGl[MAXv0];
...
int _cuts;
void process_mini_he3_v2_run10(int condor=0){
	...
	if(_condor==0){
	//fin.open("/star/u/yhleung2/pwg/33.standard_reco/mini_x.list");
	fin.open("/star/u/yhleung2/pwg/33.standard_reco/filelist_he3_run10_sampleX.list");
	//strcpy (ofile, "he3_x.root");
	//strcpy (ofile, "he3_x_etaltm04.root");
	//strcpy (ofile, "he3_x_etagtm04.root");
	//nChains = 341;
	strcpy (ofile, "he3_x_run10_v2.root");
	nChains =102;
	}
		
	if(_condor==1){
        //fin.open("/star/u/yhleung2/pwg/33.standard_reco/mini.list");
        fin.open("/star/u/yhleung2/pwg/33.standard_reco/filelist_he3_run10_sample1.list");
        //strcpy (ofile, "he3_1.root");
        //strcpy (ofile, "he3_1_etaltm04.root");
        //strcpy (ofile, "he3_1_etagtm04.root");
        //nChains = 650;
	strcpy (ofile, "he3_1_run10_v2.root");
        nChains =102;
        }
	...
	StMiniMcTree.SetBranchAddress("mMatchedPairs.mNCommonHit",&mMatchedPairsmNCommonHit);
        StMiniMcTree.SetBranchAddress("mMatchedPairs.mNAssocGl",&mMatchedPairsmNAssocGl);
	...
	StMiniMcTree.SetBranchAddress("mSplitPairs.mNPossible",&mSplitPairsmNPossible);
        StMiniMcTree.SetBranchAddress("mSplitPairs.mNAssocGl",  &mSplitPairsmNAssocGl);
...
*/
	...
	//cout<<"mMatchedPairsmPtMc:"<<i<<":"<<mMatchedPairsmPtMc[i]<<endl;
		...
		h_match_eta->Fill(mMatchedPairsmEtaGl[i],2*mMatchedPairsmPtGl[i]);
		...
		g_match_eta->Fill(mMatchedPairsmEtaGl[i],2*mMatchedPairsmPtGl[i]);
		}

		
		//if(mMatchedPairsmEtaGl[i]<-0.4){
		//if(mMatchedPairsmEtaGl[i]>-0.4){
		if(mMatchedPairsmEtaGl[i]>-99999){
...
//		}
		...
		if(mMatchedPairsmNAssocGl[i]==2){
			for(int j=0;j<mNSplitPair;j++){
				if(mMatchedPairsmPtMc[i]==mSplitPairsmPtMc[j]){
//					cout<<mMatchedPairsmNHitMc[i]<<" "<<mSplitPairsmNHitMc[j]<<endl;
//					cout<<mMatchedPairsmNCommonHit[i]<<" "<<mSplitPairsmNCommonHit[j]<<endl;			
//					cout<<mMatchedPairsmFitPts[i]<<" "<<mSplitPairsmFitPts[j]<<endl;
//					cout<<mMatchedPairsmDedxPts[i]<<" "<<mSplitPairsmDedxPts[j]<<endl;
					h_ms_mc->Fill(mMatchedPairsmNHitMc[i],mSplitPairsmNHitMc[j]);
					h_ms_common->Fill(mMatchedPairsmNCommonHit[i],mSplitPairsmNCommonHit[j]);
					h_ms_fit->Fill(mMatchedPairsmFitPts[i],mSplitPairsmFitPts[j]);
					h_ms_dedx->Fill(mMatchedPairsmDedxPts[i],mSplitPairsmDedxPts[j]);

//					cout<<endl;
//			cout<<"mMatchedPairsmPtMc:"<<mMatchedPairsmPtMc[i]<<" "<<mSplitPairsmPtMc[j]<<endl;
					}
				}
			}
		}
		...
		h_split_eta->Fill(mSplitPairsmEtaGl[i],2*mSplitPairsmPtGl[i]);
		...
		//if(mSplitPairsmEtaGl[i]<-0.4){
		//if(mSplitPairsmEtaGl[i]>-0.4){
		if(mSplitPairsmEtaGl[i]>-999999){
...
void book_histo()
{
  ...
  h_ht_chi2topo = new TH3F("h_ht_chi2topo","",20,-2.0,2.0,50,0,5,500,0,10);
  h_ht_chi2ndf = new TH3F("h_ht_chi2ndf","",20,-2.0,2.0,50,0,5,100,0,10);
  h_nhits_he = new TH3F("h_nhits_he","",20,-2.0,2.0,50,0,5,80,0,80);
  h_nhits_pi = new TH3F("h_nhits_pi","",20,-2.0,2.0,50,0,5,80,0,80);

  h_ht_dca_he = new TH3F("h_ht_dca_he","",20,-2.0,2.0,50,0,5,200,0,20);
  h_ht_dca_pi     = new TH3F("h_ht_dca_pi","",20,-2.0,2.0,50,0,5,200,0,20);

  h_ht_bdfvtx = new TH3F("h_ht_bdfvtx","",20,-2.0,2.0,50,0,5,1000,0,5);
  h_ht_bdfvtx2 = new TH3F("h_ht_bdfvtx2","",20,-2.0,2.0,50,0,5,1000,0,1);
  h_ht_lifetime = new TH3F("h_ht_lifetime","",20,-2.0,2.0,50,0,5,500,0,100);

  h_ht_bdt = new TH3F("h_ht_bdt","",20,-2.0,2.0,50,0,5,200,-1.,1.);
  h_cent = new TH1F("h_cent","", 9,0,9);

  h_bnhits = new TH3F("h_bnhits","",20,-2.0,2.0,50,0,5,70,0,70);
  h_bdedx = new TH3F("h_bdedx","",20,-2.0,2.0,50,0,5,100,0,100);
  h_pt = new TH2F("h_pt","",100,-1.5,1.5,100,0,10);
  h_r_pt = new TH2F("h_r_pt","",100,-1.5,1.5,100,0,10);

  h_eta = new TH2F("h_eta","",200,-2.,2.,50,0,5);
  g_eta = new TH2F("g_eta","",200,-2.,2.,50,0,5);
  h_match_eta = new TH2F("h_match_eta","",200,-2.,2.,50,0,5);
  g_match_eta = new TH2F("g_match_eta","",200,-2.,2.,50,0,5);
  h_split_eta = new TH2F("h_split_eta","",200,-2.,2.,50,0,5);
  g_split_eta = new TH2F("g_split_eta","",200,-2.,2.,50,0,5);

  h_match_eta_mc = new TH2F("h_match_eta_mc","",200,-2.,2.,50,0,5);
  h_split_eta_mc = new TH2F("h_split_eta_mc","",200,-2.,2.,50,0,5);

  h_phi = new TH2F("h_phi","",200,-6.5,6.5,100,0,10);
  h_rap = new TH2F("h_rap","",200,-1.5,1.5,100,0,10);
  h_dca = new TH3F("h_dca","",20,-2.0,2.0,50,0,5,500,0,20);
  ...
  h_match_nhitsfit = new TH3F("h_match_nhitsfit","",  20,-2.0,2.0,50,0,5,60,0,60);
  h_match_nhitsdedx = new TH3F("h_match_nhitsdedx","",20,-2.0,2.0,50,0,5,60,0,60);
  h_match_nhitsmax = new TH3F("h_match_nhitsmax","",  20,-2.0,2.0,50,0,5,60,0,60);
...
h_ms_mc = new TH2F("h_ms_mc","",50,0,50,50,0,50);
h_ms_common = new TH2F("h_ms_common","",50,0,50,50,0,50);
h_ms_fit = new TH2F("h_ms_fit","",50,0,50,50,0,50);
h_ms_dedx = new TH2F("h_ms_dedx","",50,0,50,50,0,50);
...
h_split_max->Sumw2();
  ...
  h_split_nhitsfit = new TH3F("h_split_nhitsfit","",  20,-2.0,2.0,50,0,5,60,0,60);
  h_split_nhitsdedx = new TH3F("h_split_nhitsdedx","",20,-2.0,2.0,50,0,5,60,0,60);
  h_split_nhitsmax = new TH3F("h_split_nhitsmax","",  20,-2.0,2.0,50,0,5,60,0,60);
