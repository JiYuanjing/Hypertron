...
//}
//
//cout<<"part.GetPDG():"<<part.GetPDG()<<" "<<vMCParticles[iMC].GetMotherId()<<" "<<part.GetDaughterIds().size()<<" "<<
...
//FOR HT embedding ONLy
        if( part.GetPDG()==211 && vMCParticles[iMC].GetMotherId()==-1 && part.GetDaughterIds().size()==2 ){
//		part.SetPDG(3004);				
		part.SetPDG(3005);				
	}
//FOR HT3body only
          if( part.GetPDG()==211 && vMCParticles[iMC].GetMotherId()==-1 && part.GetDaughterIds().size()==3){
//	cout<<"vMCParticles[part.GetDaughterIds()[0]].GetPDG():"<<vMCParticles[part.GetDaughterIds()[0]].GetPDG()<<" "<<vMCParticles[part.GetDaughterIds()[1]].GetPDG()<<" "<<vMCParticles[part.GetDaughterIds()[2]].GetPDG()<<endl;
//        if(vMCParticles[part.GetDaughterIds()[0]].GetPDG()==-211 && vMCParticles[part.GetDaughterIds()[1]].GetPDG()==211 && vMCParticles[part.GetDaughterIds()[2]].GetPDG()==2212)
...
//cout<<"xxiMC:" <<  iMC<<" pdg:"<<vMCParticles[iMC].GetPDG()<<" "<< part.GetDaughterIds().size()<<endl;
...
void KFTopoPerformance::MatchParticles()
{
  ...
  for( unsigned int iRP = 0; iRP < fTopoReconstructor->GetParticles().size(); iRP++ ) {
    ...
    //END DEBUG
      ...
      //DEBUG
      //if (rPart.GetPDG() == 3334){
      //cout<<"daughters:"<<NRDaughters<<" "<<iD<<" "<< mmId<<" "<<nClones<<endl;
      //}
      //END DEBUG
...
//                  abs( TempPart.GetPDG() ) == 500443) ) return;
  ...
  histoParameters2D[0][iParticle][4]->Fill(M,Pt,1);
...
//b 
//if(fabs(fTopoReconstructor->GetParticles()[iP].GetPDG())==3312){ 
//      cout<<"ID:"<<fTopoReconstructor->GetParticles()[iP].GetPDG()<<endl;
//}
//    cout<<"ID:"<<iP<<endl; 
