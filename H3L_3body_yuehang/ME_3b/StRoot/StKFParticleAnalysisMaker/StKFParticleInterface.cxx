...
{  
  ...
  {
    StMuMcTrack *mumcTrack = muDst->MCtrack(iMCTrack);
    if (! mumcTrack) continue;    
    KFMCTrack &mcTrackKF = mcTracks[iMCTrack];
    mumcTrack->FillKFMCTrack(mcTrackKF);
  ...
  */
    ...
    if(fVtxErr)
    {
      ...
      {
        StMuTrack *mugTrack = muDst->primaryTracks(iTrack);
        if (! mugTrack)            continue;
        const int bnHitsFit = mugTrack->nHitsFit();
        const int bnHitsPoss = mugTrack->nHitsPoss();
        const float bquality = (float)bnHitsFit/(float)bnHitsPoss;
        const float bdca = mugTrack->dcaGlobal(bestPV).mag();
  ...
  for (Int_t iTrack = 0; iTrack < nGlobalTracks; iTrack++) 
  {
    StMuTrack *mugTrack = muDst->globalTracks(iTrack);
    if (! mugTrack)            continue;
    if (! mugTrack->charge())  continue;
    if (  mugTrack->flag() < 100 ||  mugTrack->flag()%100 == 11) continue; // bad fit or short track pointing to EEMC
    if (  mugTrack->flag() > 1000) continue;  // pile up track in TPC
    if (  mugTrack->nHitsFit() < 15) continue;
    //if (  mugTrack->nHitsFit() < 20) continue;
    //if (  mugTrack->nHitsFit() < 17) continue;

    if (  mugTrack->probPidTraits().dEdxErrorFit() < 0.04 || mugTrack->probPidTraits().dEdxErrorFit() > 0.12 ) continue;

    int nHftHitsInTrack = mugTrack->nHitsFit(kIstId) + mugTrack->nHitsFit(kSsdId) + mugTrack->nHitsFit(kPxlId);
    if(fCollectTrackHistograms) fTrackHistograms[0]->Fill(nHftHitsInTrack);
    if(fUseHFTTracksOnly && nHftHitsInTrack < 3) continue;

    const int index = mugTrack->id();
    //cout<<"mugTrack->idTruth():"<<mugTrack->idTruth()<<  " "<< processSignal<<endl   ;
    mcIndices[index] = mugTrack->idTruth()-1;
    ...
    Int_t dcaGeometryIndex = mugTrack->index2Cov();
    ...
    Int_t q = 1; if (mugTrack->charge() < 0) q = -1;
    KFPTrack track;
    if( !GetTrack(*dcaG, track, q, index) ) continue;

    if(fCollectTrackHistograms)
    {
      fTrackHistograms2D[0]->Fill(track.GetP(), mugTrack->dEdx()*1.e6);
      if(q>0) fTrackHistograms2D[1]->Fill(track.GetP(), mugTrack->dEdx()*1.e6);
      else    fTrackHistograms2D[2]->Fill(track.GetP(), mugTrack->dEdx()*1.e6);  
    }


    //b
    StPhysicalHelixD iHelix = mugTrack->helix();  
    ...
    if(timeTof > 0. && lengthTof > 0.)
    {
      ...
      if(fCollectTrackHistograms)
      {
        fTrackHistograms2D[3]->Fill(track.GetP(), mugTrack->dEdx()*1.e6);
        fTrackHistograms2D[4]->Fill(track.GetP(), m2tof);
      }
    }

    double dEdXPull[7] = { fabs(mugTrack->dEdxPull(0.139570, fdEdXMode, 1)),   //0 - pi
      fabs(mugTrack->dEdxPull(0.493677, fdEdXMode, 1)),   //1 - K
      fabs(mugTrack->dEdxPull(0.938272, fdEdXMode, 1)),   //2 - p
      fabs(mugTrack->dEdxPull(1.876124, fdEdXMode, 1)),   //3 - d
      fabs(mugTrack->dEdxPull(2.809432, fdEdXMode, 1)),   //4 - t
      fabs(mugTrack->dEdxPull(2.809413, fdEdXMode, 2)),   //5 - He3
    ...
    /*                            
                                  double dEdXPull[7] = { fabs(2*mugTrack->nSigmaPion()),   //0 - pi
                                  fabs(2*mugTrack->nSigmaKaon()),   //1 - K
                                  fabs(2*mugTrack->nSigmaProton()),   //2 - p
                                  fabs(mugTrack->dEdxPull(1.876124, fdEdXMode, 1)),   //3 - d
                                  fabs(mugTrack->dEdxPull(2.809432, fdEdXMode, 1)),   //4 - t
                                  fabs(mugTrack->dEdxPull(2.809413, fdEdXMode, 2)),   //5 - He3
                                  fabs(mugTrack->dEdxPull(3.728400, fdEdXMode, 2))};  //6 - He4
    ...
    fNHitsdedx[index] = mugTrack->nHitsDedx();
    fTrackzdeuteron[index] = mugTrack->dEdxPull(1.876124, fdEdXMode, 1)*mugTrack->probPidTraits().dEdxErrorFit();
    //endb
