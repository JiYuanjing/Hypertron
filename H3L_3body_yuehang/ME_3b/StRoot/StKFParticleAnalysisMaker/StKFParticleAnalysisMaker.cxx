...
{  
  ...
  if(isGoodEvent)
  {
    ...
    if(bWriteTree){
      ...
      if (fMixEvent && fIsPicoAnalysis && isGoodEvent && EPbin>=0 && centralityBin>=0 && centralityBin<Cuts::nCentralityBins && Vtxbin>=0 && Vtxbin<Cuts::nVtxbins)
      {
        StMixEvent newEvent;
        ...
        // for (int in=0;in<4;in=in+2) 
        for (int in=0;in<1;in=in+2) 
        {
          for (int iTrack= rTracks[in].FirstProton(); iTrack<rTracks[in].LastProton() ;iTrack++)
          {
            int pdgId = rTracks[in].PDG()[iTrack];

            if (!(abs(pdgId)==2212)) continue;
            // cout << rTracks[0].PVIndex()[iTrack]<<endl;
            // if (rTracks[0].PVIndex()[iTrack]>=0) continue; //not secondary track
            // const int_v& trackPVIndex = reinterpret_cast<const  int_v&>(rTracks[0].PVIndex()[iTrack]);
            // const int_m& isTrackSecondary = (trackPVIndex < 0);
            ...
            // cout << rTracks[0].Id()[iTrack]<<endl;
            KFParticle particle; 
            fStKFParticlePerformanceInterface->GetParticle(particle, rTracks[in].Id()[iTrack]);
            // cout <<particle.DaughterIds()[0] << endl;
            float sigma, chi2prim, dca,m2;
            int nhits;
            FillDaughterInfo(particle, particle.DaughterIds()[0], 2212, chi2prim, nhits, dca, sigma, m2 );
            if (nhits<15) continue; 
            ...
            newEvent.p_v.push_back(particle);   
          }
        }

        // for (int in=1;in<4;in=in+2) 
        for (int in=1;in<2;in=in+2) 
        {
          for (int iTrack= rTracks[in].FirstPion(); iTrack<rTracks[in].LastPion() ;iTrack++)
          {
            int pdgId = rTracks[in].PDG()[iTrack];
            ...
            // cout << rTracks[in].PDG()[iTrack]<<endl;
            KFParticle particle; 
            fStKFParticlePerformanceInterface->GetParticle(particle, rTracks[in].Id()[iTrack]);
            // cout <<particle.DaughterIds()[0] << endl;
            float sigma, chi2prim, dca,m2;
            int nhits;
            FillDaughterInfo(particle, particle.DaughterIds()[0], 211, chi2prim, nhits, dca, sigma, m2 );
            if (nhits<15) continue; 
            ...
            newEvent.pi_v.push_back(particle);   
          }
        }
          // cout <<"p "<<newEvent.p_v.size()<<" "<<newEvent.p_v_dca.size() << endl;
          // cout <<"d "<<newEvent.d_v.size()<<" "<<newEvent.d_v_dca.size() << endl;
          // cout <<"pi "<<newEvent.pi_v.size()<<" "<<newEvent.pi_v_dca.size() << endl;
        ...
        TVector3 dV0(0,0,0);
        bool fillBuffer = newEvent.p_v.size()>0 && newEvent.pi_v.size()>0 && newEvent.d_v.size()>0;
        if (fillBuffer) {
          ...
          for (int id = 0;id<newEvent.d_v.size();id++){
            for (int iP = 0;iP<newEvent.p_v.size();iP++){
              for (int iPi = 0;iPi<newEvent.pi_v.size();iPi++){
                // cout << " start fill mix tree: "<<  << "  "<< iP<< endl;
                bool filltree = FillThreeDaughtersMix(newEvent.pi_v[iPi],newEvent.p_v[iP],newEvent.d_v[id], dV0,dV0,dV0);
                ...
                chi2primary_pi = newEvent.pi_v_chi2prim[iPi];
                bpionnsigma = newEvent.pi_v_sigma[iPi];
                nhits_pion = newEvent.pi_v_nhits[iPi];
                dca_pion = newEvent.pi_v_dca[iPi];
                bpionm2 = newEvent.pi_v_m2[iPi];

                chi2primary_proton = newEvent.p_v_chi2prim[iP];
                bprotonsigma = newEvent.p_v_sigma[iP];
                nhits_proton = newEvent.p_v_nhits[iP];
                dca_proton = newEvent.p_v_dca[iP];
                bprotonm2 = newEvent.p_v_m2[iP];    
                if (filltree) 
                {
                  htriton3_tree->Fill();
                } 
        ...
        if ( bMixEventBuffer[centralityBin][EPbin][Vtxbin].size()==fMixEventBufferSize && fillBuffer){
          ...
          bisMix = 1;
          bismc = 0;
          float dVx1 = newEvent.Vx - bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].Vx;
          float dVy1 = newEvent.Vy - bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].Vy;
          float dVz1 = newEvent.Vz - bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].Vz;
          TVector3 dV1(dVx1, dVy1, dVz1);

          for (int ie =1;ie<fMixEventBufferSize;ie++){ 
            ...
            float dVx2 = newEvent.Vx - bMixEventBuffer[centralityBin][EPbin][Vtxbin][ie].Vx;
            float dVy2 = newEvent.Vy - bMixEventBuffer[centralityBin][EPbin][Vtxbin][ie].Vy;
            float dVz2 = newEvent.Vz - bMixEventBuffer[centralityBin][EPbin][Vtxbin][ie].Vz;
            TVector3 dV2(dVx2, dVy2, dVz2);

            for (int id = 0;id<bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].d_v.size();id++){
              for (int ip = 0;ip<bMixEventBuffer[centralityBin][EPbin][Vtxbin][ie].p_v.size();ip++){
                for (int ipi = 0;ipi<newEvent.pi_v.size();ipi++){
                  //will update here!
                  bool filltree = FillThreeDaughtersMix( newEvent.pi_v[ipi],bMixEventBuffer[centralityBin][EPbin][Vtxbin][ie].p_v[ip],bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].d_v[id], dV0, dV2, dV1 );
                  if (filltree) 
                  {
                    //add daughter further info
                    nhits_deuteron = bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].d_v_nhits[id];
                    dca_deuteron = bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].d_v_dca[id];
                    bdm2 = bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].d_v_m2[id];
                    chi2primary_d = bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].d_v_chi2prim[id];
                    bdedx = bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].d_v_dEdx[id];
                    bzdeuteron =  bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].d_v_z[id];
                    ...
                    chi2primary_proton = bMixEventBuffer[centralityBin][EPbin][Vtxbin][ie].p_v_chi2prim[ip];
                    bprotonsigma = bMixEventBuffer[centralityBin][EPbin][Vtxbin][ie].p_v_sigma[ip];
                    nhits_proton = bMixEventBuffer[centralityBin][EPbin][Vtxbin][ie].p_v_nhits[ip];
                    dca_proton = bMixEventBuffer[centralityBin][EPbin][Vtxbin][ie].p_v_dca[ip];
                    bprotonm2 = bMixEventBuffer[centralityBin][EPbin][Vtxbin][ie].p_v_m2[ip];
            ...
            for (int id = 0;id<newEvent.d_v.size();id++){
              for (int ip = 0;ip<bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].p_v.size();ip++){
                for (int ipi = 0;ipi<bMixEventBuffer[centralityBin][EPbin][Vtxbin][ie].pi_v.size();ipi++){
                  bool filltree = FillThreeDaughtersMix(bMixEventBuffer[centralityBin][EPbin][Vtxbin][ie].pi_v[ipi],bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].p_v[ip],newEvent.d_v[id], dV2, dV1, dV0 );
                  //add daughter further info
                  chi2primary_d = newEvent.d_v_chi2prim[id];
                  nhits_deuteron = newEvent.d_v_nhits[id];
                  dca_deuteron = newEvent.d_v_dca[id];
                  ...
                  bzdeuteron =  newEvent.d_v_z[id];
                  //     
                  chi2primary_pi = bMixEventBuffer[centralityBin][EPbin][Vtxbin][ie].pi_v_chi2prim[ipi];
                  bpionnsigma = bMixEventBuffer[centralityBin][EPbin][Vtxbin][ie].pi_v_sigma[ipi];
                  nhits_pion = bMixEventBuffer[centralityBin][EPbin][Vtxbin][ie].pi_v_nhits[ipi];
                  dca_pion = bMixEventBuffer[centralityBin][EPbin][Vtxbin][ie].pi_v_dca[ipi];
                  bpionm2 = bMixEventBuffer[centralityBin][EPbin][Vtxbin][ie].pi_v_m2[ipi];

                  chi2primary_proton = bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].p_v_chi2prim[ip];
                  bprotonsigma = bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].p_v_sigma[ip];
                  nhits_proton = bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].p_v_nhits[ip];
                  dca_proton = bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].p_v_dca[ip];
                  bprotonm2 = bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].p_v_m2[ip];
                  if (filltree) 
                  {
            ...
            for (int id = 0;id<bMixEventBuffer[centralityBin][EPbin][Vtxbin][ie].d_v.size();id++){
              for (int ip = 0;ip<newEvent.p_v.size();ip++){
                for (int ipi = 0;ipi<bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].pi_v.size();ipi++){
                  bool filltree = FillThreeDaughtersMix(bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].pi_v[ipi],newEvent.p_v[ip],bMixEventBuffer[centralityBin][EPbin][Vtxbin][ie].d_v[id], dV1, dV0, dV2 );
                  //add daughter further info
                  ...
                  //     
                  chi2primary_pi = bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].pi_v_chi2prim[ipi];
                  bpionnsigma = bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].pi_v_sigma[ipi];
                  nhits_pion = bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].pi_v_nhits[ipi];
                  dca_pion = bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].pi_v_dca[ipi];
                  bpionm2 = bMixEventBuffer[centralityBin][EPbin][Vtxbin][0].pi_v_m2[ipi];
...
bool StKFParticleAnalysisMaker::FillThreeDaughtersMix(KFParticle pion, KFParticle proton, KFParticle deuteron, TVector3 dV_pi, TVector3 dV_p, TVector3 dV_d )
{
  if (!(pion.GetPDG()<0 && proton.GetPDG()>0 && deuteron.GetPDG()>0)) return false; //only look at particle
  float fDistanceCut=5, fLCut=1, fchi2Cut=10;
  float cut[3]={3,10,10};  // ldl, chi2topo, chi2geo

  deuteron.X()=dV_d.Z()+deuteron.GetX();
  deuteron.Y()=dV_d.Z()+deuteron.GetY();
  deuteron.Z()=dV_d.Z()+deuteron.GetZ();

  pion.X()=dV_pi.X()+pion.GetX();
  pion.Y()=dV_pi.Y()+pion.GetY();
  pion.Z()=dV_pi.Z()+pion.GetZ();

  proton.X()=dV_p.X()+proton.GetX();
  proton.Y()=dV_p.Y()+proton.GetY();
  proton.Z()=dV_p.Z()+proton.GetZ();

  const KFParticle* v_d01[2] = {&pion, &proton};
  KFParticle v_lambda;
  v_lambda.Construct(v_d01, 2, 0);
  float m, dm;
  float_v vl, vdl;
  v_lambda.GetMass(m,dm);
  v_lambda_mass_0 = m;
  if (m>1.15 || m<1.05) return false;

  KFParticleSIMD tempLambdaSIMD(v_lambda);
  KFParticleSIMD pv(fStKFParticleInterface->GetTopoReconstructor()->GetPrimVertex());
  tempLambdaSIMD.GetDistanceToVertexLine(pv, vl, vdl);
  v_lambda_ldl_0 = vl[0]/vdl[0];   //>3
  v_lambda_l_0 = vl[0];   //>1
  //v_lambda.GetDistanceToVertexLine(fStKFParticleInterface->GetTopoReconstructor()->GetPrimVertex(), vl, vdl);
  // v_lambda_ldl_0 = vl/vdl;
  v_lambda_chi2primary_0 = v_lambda.GetDeviationFromVertex(fStKFParticleInterface->GetTopoReconstructor()->GetPrimVertex());
  if (v_lambda_ldl_0<cut[0]) return false;
  if (v_lambda_l_0<fLCut) return false;
  if ((tempLambdaSIMD.Chi2()/static_cast<float_v>(tempLambdaSIMD.NDF()) < float_v(fchi2Cut) ).isEmpty()) return false;
  if (KFPMath::Finite(tempLambdaSIMD.GetChi2()).isEmpty()) return false;
  if ((tempLambdaSIMD.GetChi2() > 0.0f).isEmpty()) return false;
  if ((tempLambdaSIMD.GetChi2() == tempLambdaSIMD.GetChi2()).isEmpty()) return false;
