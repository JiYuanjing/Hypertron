...
class StKFParticleInterface: public TObject
{
 public:
  ...
  bool ProcessEvent(StMuDst* muDst, std::vector<int>& goodTracks, bool processSignal);
  ...
  const float GetM2tof(const int trackId)  const { return fM2tof[trackId]; }
