...
bool StKFParticleInterface::IsGoodPV(const KFVertex& pv)
{
  ...
  isGoodPV = (pv.Z() > 198) && (pv.Z() < 202) &&
  (sqrt(pv.X()*pv.X() + (pv.Y()+2)*(pv.Y()+2)) < 1.5);
  return isGoodPV;
}
...
std::vector<int> StKFParticleInterface::GetPID(double m2, double p, int q, double dEdX, double dEdXPull[7], bool isTofm2, const int trackId)
{
   ...
   */
  {
      ...
      //if(p>0.5)
    {
      ...
      double lowerParameters[4]={24.7439,-1.09225,0.391905,-0.00815047};      //lower 1.5 sigma band
...
bool StKFParticleInterface::ProcessEvent(StPicoDst* picoDst, std::vector<int>& triggeredTracks)
{
  ...
  for (Int_t iTrack = 0; iTrack < nGlobalTracks; iTrack++)
  {
    ...
    if (  gTrack->nHitsDedx() < 5) continue; // def: 5
    if (  gTrack->nHitsFit() < 15) continue;
    if (  gTrack->nHitsFit() < 0.52 * gTrack->nHitsMax() )  continue;
    if (  gTrack->dEdxError() < 0.04 || gTrack->dEdxError() > 0.12 ) continue;
    
    //        float dca = gTrack->gDCA( picoPV ).Mag();
    //        if( fabs(dca) > 3.0 ) continue;
    
    float gpt   = gTrack->gMom().Perp();
    if( gpt < 0.15 || gpt > 10. ) continue;
    
    float geta  = gTrack->gMom().PseudoRapidity();
    if( geta < -2.0 || geta > 0 ) continue;
    ...
    {
      fTrackHistograms2D[0]->Fill(gTrack->gMom().Perp(), gTrack->dEdx());
      if(q>0) fTrackHistograms2D[1]->Fill(gTrack->gMom().Perp(), gTrack->dEdx());
      else    fTrackHistograms2D[2]->Fill(gTrack->gMom().Perp(), gTrack->dEdx());
    ...
    if(gTrack->bTofPidTraitsIndex() > 0)
    {
      ...
      if(fCollectTrackHistograms)
      {
        fTrackHistograms2D[3]->Fill(gTrack->gMom().Perp(), gTrack->dEdx());
        fTrackHistograms2D[4]->Fill(gTrack->gMom().Perp(), m2tof);
      }
    }
  ...
  if(fRotation){
    for(int iTr=0; iTr<nPartSaved; iTr++){
      if(fParticlesPdg[iTr]==fRotationPID || fParticlesPdg[iTr]==-fRotationPID){
        //fParticles[iTr].RotateXY(fRotationAngle, vtxb); //rotate 180 degrees
        //fParticles[iTr].RotateXY(fRotationAngle * randomNumber.Uniform(0.0, 1.0), vtxb); //rotate random angle between 0 to 180 degress
        fParticles[iTr].RotateXY(fRotationAngle * (5.0/6.0 + randomNumber.Uniform(0.0, 1.0)/3.0), vtxb); //rotate random angle between 150 to 210 degrees
        
        //cout << "Random angle: " << fRotationAngle * randomNumber.Uniform(0.0, 1.0) << endl;
      }
    }
  }
  //endb
