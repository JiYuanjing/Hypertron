...
void readtree(TString mInputlist="Lambda_tree_mc.root", int const mode = 1,   TString outfile="fout_Lambda.root", int const mcH3L=1)
{
  double snn = 3;
  double ycm;
  if(snn==3){
...
/*  if (mode==0) fgpt_0= new TFile("ht_input_mc_fine_v20.root","READ"); */
 if (mode==0 && mcH3L==1) fgpt_0= new TFile("fH3L_phase_quasi_wt.root","READ");
  if (mode==0 && mcH3L==1 && mInputlist.Contains("phase")) fgpt_0= new TFile("fH3L_phase_phase_wt.root","READ");  //phase space decay
/*  if (mode==1) fgpt_0 = new TFile("ld_input_mc_fine_v20.root","READ");//more stats */
 if (mode==1 || (mcH3L==-20 && mode==0)) fgpt_0 = new TFile("flambda_phase_wt.root","READ");//more stats
/*  TH2F*  g_pt_fine_in = (TH2F*)fgpt_0->Get("g_pt_fine")->Clone("g_pt_fine_in"); */
 TH2F*  g_pt_fine_in = (TH2F*)fgpt_0->Get("hPhase")->Clone("g_pt_fine_in");
  ...
  for (int i=0;i<n_lambda_Entries;i++)
  {
    ...
    if ( !(bismc == mcH3L)  ) continue;
    TLorentzVector mcptc;
    if (mode==0 && mcH3L==1 && bismc ==mcH3L) 
      mcptc.SetXYZM(bmcpx,bmcpy,bmcpz,ht_mass);
    else if (mode==0 && mcH3L==-20 && bismc == mcH3L) {
      TLorentzVector pion;
    ...
    if (mode==0 && mcH3L ==1) ptweight = bolt1->Eval(mcMotherPt);
    else if (mode==1 || mcH3L==-20){
    ...
    double mcweight = 1./g_pt_fine_in->GetBinContent(g_pt_fine_in->FindBin(bmcrap,mcMotherPt));;
    reweight=1;
    ...
    if ((mode==0 && bismc==mcH3L) || (mode==1 && bismc==1) ){ 
      ...
      if (mode==0) hptppimass->Fill( ppi_pt, mass_01, weight);
      else if (mode==1) hptppimass->Fill( ppi_pt, bparticlemass, weight);
    ...
    if (mode==0 && bismc==mcH3L ) {
      ...
      /* weight=1; */
      /* bool passTopoCuts=1; */
      bool passTopoCuts =  ht_l >8 && ht_ldl>10 && dca_pion>1.5 && dca_proton>0.5 && dca_proton<5 && dca_deuteron<2 && ht_chi2topo<10 && ht_chi2ndf<4.0;
